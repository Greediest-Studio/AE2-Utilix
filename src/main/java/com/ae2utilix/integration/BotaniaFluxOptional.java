...
public final class BotaniaFluxOptional {
    ...
    public static long insertNetwork(IStorageGrid storage, IEnergySource energy,
        ...
        if (amount <= 0 || storage == null || energy == null || source == null) return 0;
        try {
            if (type == MANA) {
                IMEInventory<ManaStack> inventory = storage.getInventory(ManaStorageChannel.INSTANCE);
                if (inventory == null) return 0;
                ManaStack remainder = Platform.poweredInsert(energy, inventory,
                        new ManaStack(amount), source, mode);
            ...
            if (type == FE) {
                ...
                if (inventory == null) return 0;
                FluxStack remainder = Platform.poweredInsert(energy, inventory,
                        new FluxStack(amount), source, mode);
                return amount - (remainder == null ? 0 : remainder.getStackSize());
            }
        } catch (Exception ignored) {
        }
    ...
    public static long extractNetwork(IStorageGrid storage, IEnergySource energy,
        ...
        try {
            if (type == MANA) {
                IMEInventory<ManaStack> inventory = storage.getInventory(ManaStorageChannel.INSTANCE);
                if (inventory == null) return 0;
                ManaStack extracted = Platform.poweredExtraction(energy, inventory,
                        new ManaStack(amount), source, mode);
            ...
            if (type == FE) {
                IMEInventory<FluxStack> inventory = storage.getInventory(FluxStorageChannel.INSTANCE);
                if (inventory == null) return 0;
                FluxStack extracted = Platform.poweredExtraction(energy, inventory,
                        new FluxStack(amount), source, mode);
                return extracted == null ? 0 : extracted.getStackSize();
            }
        } catch (Exception ignored) {
