 ...
 */
public final class MekanismEnergisticsOptional {
    ...
    private MekanismEnergisticsOptional() {
    }
    ...
    /** Returns the amount accepted by the optional MekEng storage network. */
    public static int insertGasToNetwork(IStorageGrid storage, IEnergySource energy,
            IActionSource source, GasStack stack, boolean simulate) {
        if (!isAvailable() || storage == null || energy == null || source == null
                || stack == null || stack.getGas() == null || stack.amount <= 0) {
            return 0;
        }
        try {
            IMEInventory<IAEGasStack> inventory = storage.getInventory(
                    AEApi.instance().storage().getStorageChannel(IGasStorageChannel.class));
            if (inventory == null) return 0;
            IAEGasStack input = AEGasStack.of(stack.copy());
            IAEGasStack remainder = Platform.poweredInsert(energy, inventory, input, source,
                    simulate ? Actionable.SIMULATE : Actionable.MODULATE);
            return remainder == null ? stack.amount
                    : Math.max(0, Math.min(stack.amount, stack.amount - (int) remainder.getStackSize()));
    ...
    @Nullable
    public static GasStack extractGasFromNetwork(IStorageGrid storage, IEnergySource energy,
            IActionSource source, Gas gas, int amount, boolean simulate) {
        if (!isAvailable() || storage == null || energy == null || source == null
                || gas == null || amount <= 0) return null;
        try {
            IMEInventory<IAEGasStack> inventory = storage.getInventory(
                    AEApi.instance().storage().getStorageChannel(IGasStorageChannel.class));
            if (inventory == null) return null;
            IAEGasStack extracted = Platform.poweredExtraction(energy, inventory,
                    AEGasStack.of(new GasStack(gas, amount)), source,
                    simulate ? Actionable.SIMULATE : Actionable.MODULATE);
    ...
    private static IAEGasStack extractFromNetwork(TileCommonInterfaceAlternate tile, String gasName, int amount) {
        return extractFromNetwork(tile, gasName, amount, Actionable.MODULATE);
    }

    @Nullable
    private static IAEGasStack insertIntoNetwork(TileCommonInterfaceAlternate tile, String gasName, int amount) {
        return insertIntoNetwork(tile, gasName, amount, Actionable.MODULATE);
    }
